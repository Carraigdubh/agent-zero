...
class BackupService:
    """
    Core backup and restore service for Father Ted.
    ...
    def __init__(self):
        ...
        self.agent_zero_root = files.get_abs_path("")  # Resolved Father Ted root
    ...
    def _get_default_patterns(self) -> str:
        ...
        Only includes Father Ted project directory patterns.
        """
        # Ensure paths don't have double slashes
        agent_root = self.agent_zero_root.rstrip('/')

        return f"""# Father Ted Knowledge (excluding defaults)
{agent_root}/knowledge/**
!{agent_root}/knowledge/default/**

# Father Ted Instruments (excluding defaults)
...
{agent_root}/tmp/uploads/**"""
    ...
    def _get_agent_zero_version(self) -> str:
        """Get current Father Ted version"""
        try:
    ...
    def _translate_patterns(self, patterns: List[str], backup_metadata: Dict[str, Any]) -> List[str]:
        ...
        Replaces the backed up Father Ted root path with the current Father Ted root path
